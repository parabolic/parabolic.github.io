...
Afterwads running `terraform apply` against this template, a JSON encoded file with the content of the local variable `map` will be created in the same folder. That is easy enough.

But what if I needed a YAML encoded file? I am also not sure how it will look alike. I prefer JSON, but I also like to have some comments.

I can check out on the fly by firing up Terraform console in the same folder and just YAML encoding the local `map` variable:
...
EOT
```
Voila! We see how our file will look like if we've used the `yamlencode` function instead.
...
EOT
```
Or just "trying" out the `try` function to see how it exactly works.
...
I digress. The flexibility that the `terraform console` provides when testing, troubleshooting, or simply playing around is immense.

## [Shell Autocomplete]

One of the commands that do not appear when invoking terraform via the CLI. Myself having worked with Terraform for quite a while, I stumbled upon this command fairly recently. IT lets you use shell tab-completion. As of writing this blog post, it works with `bash` or `zsh` shells only.
...
Usually, I edit the state via Terraform CLI. Sometimes, though, it is required that I need to manually edit it. I do it via a pull, edit, and push cycle. These commands will work regardless of the type of backend configured.
...
The changes that Terraform will report attesting to the successful manual state edit.
...
Getting to know a specific technology or a tool is not as easy as it seems. Hopefully, this blog post will make some of terraform unusual options clearer and more usable.
